#include "Window.hpp"
Window::Window(int width, int height, const char* title) : window(glfwCreateWindow(width, height, title, NULL, NULL)), scale(100.0f), keyStatus(GLFW_RELEASE), player(NULL) {
...
Window::operator bool() {
	...
	if (glfwGetKey(window, GLFW_KEY_A) || glfwGetKey(window, GLFW_KEY_LEFT)) { //左側を向いたベクトル(x,z)を求める
		direction[0] += *(player->getDirection() + 2);
		direction[1] += -*(player->getDirection());
	}
	if (glfwGetKey(window, GLFW_KEY_W) || glfwGetKey(window, GLFW_KEY_UP)) { //同様
		direction[0] += *(player->getDirection());
		direction[1] += *(player->getDirection() + 2);
	}
	if (glfwGetKey(window, GLFW_KEY_D) || glfwGetKey(window, GLFW_KEY_RIGHT)) { //同様
		direction[0] += -*(player->getDirection() + 2);
		direction[1] += *(player->getDirection());
	}
	if (glfwGetKey(window, GLFW_KEY_S) || glfwGetKey(window, GLFW_KEY_DOWN)) { //同様
		direction[0] += -*(player->getDirection());
		direction[1] += -*(player->getDirection() + 2);
	}
	if (glfwGetKey(window, GLFW_KEY_SPACE) || getMouseButton(GLFW_MOUSE_BUTTON_2) != GLFW_RELEASE) player->jump();
