...
Object::Object(GLint size, GLsizei vertex_count, const Vertex *vertex, GLsizei index_count, const GLuint *index) {
...
Object::Object(GLint size, GLsizei vertex_count, const TextureVertex* vertex, GLsizei index_count, const GLuint* index) {
	//頂点配列オブジェクトを作成(数,indexを格納する配列)
	glGenVertexArrays(1, &vao);
	//頂点配列オブジェクトを結合(オブジェクトのindex)
	glBindVertexArray(vao);
	...
	//頂点の情報をメモリに書き込む
	glBufferData(GL_ARRAY_BUFFER, vertex_count * sizeof(TextureVertex), vertex, GL_STATIC_DRAW);
	//結合されている頂点バッファオブジェクトをin変数から参照できるようにする
	glVertexAttribPointer(0, size, GL_FLOAT, GL_FALSE, sizeof(TextureVertex), static_cast<TextureVertex*>(0)->position); //座標
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(TextureVertex), static_cast<TextureVertex*>(0)->UV);       //色
	//変数を有効にする
	glEnableVertexAttribArray(0);
	glEnableVertexAttribArray(1);
