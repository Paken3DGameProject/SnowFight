...
Matrix::Matrix(Player player) {
    GLfloat look[] = {*player.getPosition() + *player.getDirection(),
...
Matrix Matrix::operator*(const Matrix &m) const {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        t.matrix[i] =
                matrix[0 + j] * m.matrix[k + 0] + matrix[4 + j] * m.matrix[k + 1] + matrix[8 + j] * m.matrix[k + 2] +
...
Matrix Matrix::orthogonal(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat z_near,
                          GLfloat z_far) { //直交投影変換行列の生成
    Matrix t;
...
Matrix
Matrix::frustum(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat z_near, GLfloat z_far) { //透視投影変換行列の生成
    Matrix t;
    const GLfloat dx(right - left);
    const GLfloat dy(top - bottom);
    const GLfloat dz(z_far - z_near);
    ...
    if (dx != 0.0f && dy != 0.0f && dz != 0.0f) {
        t.loadIdentity();
        t.matrix[0] = 2.0f * z_near / dx;
        t.matrix[5] = 2.0f * z_near / dy;
        t.matrix[8] = (right + left) / dx;
        t.matrix[9] = (top + bottom) / dy;
        t.matrix[10] = -(z_far + z_near) / dz;
        t.matrix[11] = -1.0f;
        t.matrix[14] = -2.0f * z_far * z_near / dz;
        t.matrix[15] = 0.0f;
    }
...
Matrix Matrix::perspective(GLfloat fovy, GLfloat aspect, GLfloat z_near, GLfloat z_far) { //画角と縦横比に基づく透視投影変換行列
    Matrix t;
    const GLfloat dz(z_far - z_near);
